...
const initialRelations: Relation[] = [
  ...
  },
  {
    ...
    triggerField: "專案名稱",
    targetField: "專案收支",
    autoFillFields: "專案名稱、收支金額、發票日期、備註",
    status: "啟用",
    note: "專案頁可看到收支彙總"
  }
];
...
export default function FormRelationsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"list" | "test" | "fields" | "create">("list");
  ...
  return (
    <AppLayout title="表單關聯">
      <PageHeader
        title="表單關聯"
        description="設定不同表單之間的資料帶入規則，例如客戶資料自動帶入訂單，並顯示過去訂購紀錄。"
        action={
      ...
      <div className="mb-5 grid gap-4 md:grid-cols-4">
        ...
        <Stat label="來源表單" value="5 種" icon={<Database className="h-5 w-5" />} />
        ...
        <>
          ...
          <div className="card">
            <div className="mb-5 flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold">關聯列表</h2>
                <p className="mt-1 text-sm text-slate-500">操作欄固定在左邊，編輯、刪除不用滑到最右側。</p>
              </div>
      ...
      {activeTab === "fields" && (
        <div className="grid gap-5 xl:grid-cols-[1fr_360px]">
          ...
          <div className="space-y-5">
            ...
            <div className="card">
              <h2 className="text-xl font-bold">功能保留</h2>
              <div className="mt-4 space-y-3">
                <Info title="欄位設定" text="保留顯示、必填、可編輯、排序、寬度與刪除欄位。" />
                <Info title="編輯／刪除" text="關聯列表每筆資料都保留編輯與刪除。" />
                <Info title="測試帶入" text="可示範客戶資料自動帶入訂單與過去訂購紀錄。" />
              </div>
            </div>
          </div>
        </div>
      ...
      {activeTab === "create" && (
        <div className="card">
          <div className="flex min-h-[260px] flex-col items-center justify-center text-center">
            ...
            <h2 className="text-xl font-bold">新增表單關聯</h2>
